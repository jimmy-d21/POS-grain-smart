...
export function InventoryPage() {
  ...
  const getStockStatus = (item) => {
    ...
    if (percentage <= 100) return { label: 'Low Stock', variant: 'destructive' };
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Track and manage raw materials and supplies</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className={lowStockCount > 0 ? 'border-destructive' : ''}>
          ...
          <CardContent>
            <div className={`text-3xl font-bold ${lowStockCount > 0 ? 'text-destructive' : ''}`}>{lowStockCount}</div>
          </CardContent>
        </Card>
                      ...
                      <TableRow key={item.id}>
                        ...
                        <TableCell>
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => setRestockDialog({ open: true, item })}
                          >
      ...
      {/* Restock Dialog */}
      <Dialog open={restockDialog.open} onOpenChange={(open) => setRestockDialog({ open, item: open ? restockDialog.item : null })}>
        <DialogContent>
          <DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-semibold">{restockDialog.item.currentStock} {restockDialog.item.unit}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">Reorder Level</p>
                  <p className="font-semibold">{restockDialog.item.reorderLevel} {restockDialog.item.unit}</p>
                </div>
              </div>

              <div>
                <Label htmlFor="restock-amount">Restock Amount ({restockDialog.item.unit})</Label>
                <Input
                  id="restock-amount"
                  type="number"
              ...
              {restockAmount && !isNaN(parseFloat(restockAmount)) && (
                <div className="p-4 bg-primary/10 rounded-lg">
                  ...
                  <p className="text-2xl font-bold text-primary">
                    {restockDialog.item.currentStock + parseFloat(restockAmount)} {restockDialog.item.unit}
                  </p>
                </div>
              )}
            </div>
