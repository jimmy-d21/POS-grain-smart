...
export function DashboardPage() {
  ...
  todayTransactions.forEach(transaction => {
    transaction.items.forEach(item => {
      ...
      categoryData.set(category, (categoryData.get(category) || 0) + item.subtotal);
    });
  });

  const categoryChartData = Array.from(categoryData.entries()).map(([name, value]) => ({
    name,
    value,
  }));
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground">Overview of store performance and metrics</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold text-primary">₱{todaySales.toFixed(2)}</div>
            <p className="text-xs text-muted-foreground mt-1">{todayOrders} orders today</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold text-destructive">{lowStockItems.length}</div>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={hourlyData}>
                ...
                <Tooltip />
                <Legend />
                <Line type="monotone" dataKey="sales" stroke="#2d5f3f" name="Sales (₱)" strokeWidth={2} />
                <Line type="monotone" dataKey="orders" stroke="#8b6f47" name="Orders" strokeWidth={2} />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={bestSellers}>
              ...
              <Tooltip />
              <Legend />
              <Bar yAxisId="left" dataKey="count" fill="#2d5f3f" name="Quantity Sold" />
              <Bar yAxisId="right" dataKey="revenue" fill="#8b6f47" name="Revenue (₱)" />
            </BarChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
              ...
              {lowStockItems.map(item => (
                <div key={item.id} className="flex justify-between items-center p-3 bg-destructive/10 rounded-lg">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-muted-foreground">Reorder at {item.reorderLevel} {item.unit}</p>
                  </div>
                </div>
              ))}
            </div>
